...
xception_path = os.getenv('XCEPTION_MODEL_PATH')
custom_cnn_path = os.getenv('CUSTOM_CNN_MODEL_PATH')
augmented_model_path = os.getenv('DATA_AUGMENTED_CNN_MODEL_PATH')
...
def generate_explanation_by_groq_api(image_path, prompt):
    ...
    # Encode image for suitabe input to LLM
    ...
    chat_completion = client.chat.completions.create(
        ...
        model="llama-3.2-11b-vision-preview",    # can scale it, update code to add choices.
    )
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if llm_choice == "Google Gemini-1.5-Flash":
            explanation_of_results = generate_explanation_by_gemini(saliency_map_path, prompt_to_llm)
        else:
            explanation_of_results = generate_explanation_by_groq_api(saliency_map_path, prompt_to_llm)
